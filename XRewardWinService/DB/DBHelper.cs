...
namespace Spareio.WinService.DB
{
    public class DBHelper
    {
        ...
        //{
        //    int result = 0;
        //    if (CreateDBDirectory(mineTable) == false) return result;

        //    // Open database (or create if not exits)
        //    using (var db = new LiteDatabase(dbPath + "\\xreward.db"))
        //    {
        //        var xRewadModelDetails = db.GetCollection<XRewardModel>(mineTable);
        ...
        //            using (var db = new LiteDatabase(dbPath + "\\xreward.db"))
        //            {
        //                var xRewardDetails = db.GetCollection<XRewardModel>(mineTable);
